...
use ouch_connect_nonblocking::prelude::{
    asserted_short_name, ConnectionId, ConnectionStatus, PoolConnectionStatus, SendNonBlocking, SendStatus, SvcOuch as SvcOuchRs, SvcOuchProtocolAuto, SvcOuchProtocolManual, SvcOuchSender, SvcOuchSenderRef, UserName,
};
...
impl SvcManual {
    #[new]
    fn new(host: String, callback: PyObject, max_connections: Option<NonZeroUsize>, io_timeout: Option<f64>, name: Option<&str>) -> Self {
    ...
    fn is_connected(&mut self, io_timeout: Option<f64>) -> bool {
        let io_timeout = timeout_selector(io_timeout, self.io_timeout);
        self.sender.is_next_connected_busywait_timeout(io_timeout)
    }
}
...
impl SvcAuto {
    #[new]
    fn new(host: String, callback: PyObject, usr: &str, pwd: &str, session: &str, svc_max_hbeat_interval: f64, max_connections: Option<NonZeroUsize>, io_timeout: Option<f64>, name: Option<&str>) -> Self {
        let max_connections = max_connections.unwrap_or(NonZeroUsize::new(1).unwrap());
        let callback = PyProxyCallback::new_ref(callback);
        ...
        let sender = SvcOuchRs::bind(host.as_str(), max_connections, callback, protocol, name).unwrap().into_sender_with_spawned_recver_ref();
        Self { sender, io_timeout }
    }
    fn __repr__(&mut self) -> String {
        let is_connected = self.sender.is_next_connected();
    ...
    //     let io_timeout = timeout_selector(io_timeout, self.io_timeout);
    //     py.allow_threads(move || self.sender.is_next_connected_busywait_timeout(io_timeout))
    // }
}
